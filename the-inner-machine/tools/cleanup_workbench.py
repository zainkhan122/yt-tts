...
def main():
 ...
 for p in ROOT.rglob('*'):
  ...
  is_active_media=p.suffix.lower() in protected_media and 'plan' in p.parts
  if (p.suffix.lower() in {'.wav','.mp3','.m4a','.opus','.flac','.zip','.part'} or any(x in p.parts for x in {'.work','.render_work','__pycache__'})) and not is_active_media:
